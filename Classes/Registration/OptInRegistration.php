 ...
 */
class OptInRegistration //extends AbstractRegistration
{
     ...
     * @const string
     */
    const SIGNAL_AFTER_REGISTRATION_COMPLETED = 'afterUserRegisterGrant';
     ...
     * @const string
     */
    const SIGNAL_AFTER_REGISTRATION_CANCELED = 'afterUserRegisterDenial';
     ...
     * @const string
     */
    const SIGNAL_AFTER_CREATING_OPTIN_NEW_USER = 'afterCreatingOptinUser';
     ...
     * @const string
     */
    const SIGNAL_AFTER_REGISTRATION_ENDED = 'afterDeletingUser';
    ...
    : int {
        ...
        if (
            ...
            $this->signalSlotDispatcher->dispatch(
                __CLASS__,
                self::SIGNAL_AFTER_REGISTRATION_CANCELED,
                [$frontendUser]
            );
        ...
        if ($status === 1) {
            ...
            if ($frontendUser = $this->frontendUserRepository->findByUidAlsoInactiveNonGuest($register->getUser())) {
                ...
                if ($frontendUser->getDisable()) {
                    ...
                    $this->signalSlotDispatcher->dispatch(
                        __CLASS__,
                        self::SIGNAL_AFTER_REGISTRATION_COMPLETED,
            ...
            if ($register->getCategory()) {
                ...
                $this->signalSlotDispatcher->dispatch(
                    __CLASS__,
                    self::SIGNAL_AFTER_REGISTRATION_COMPLETED . ucfirst($register->getCategory()),
     ...
     *
     * @param RKW\RkwRegistration\Domain\Model\FrontendUser|array $userData A FrontendUser object or an array with equivalent key names to frontendUser properties. E.g. "mail" oder "username"
        ...
        if ($frontendUserDatabase = $frontendUserRepository->findOneByEmailOrUsernameAlsoInactive($frontendUser->getUsername())) {
            ...
            if ($additionalData) {
                ...
                if (
                    ...
                    && (!$frontendUserRegister->isUsernameUnique())
                ) {
            ...
            if ($enable) {
                ...
                $signalSlotDispatcher->dispatch(
                    __CLASS__,
                    self::SIGNAL_AFTER_CREATING_OPTIN_NEW_USER . ucfirst($category),
     ...
     */
    protected function check(Registration $register, string $tokenYes, string $tokenNo): int
    {
    ...
    public function delete(FrontendUser $frontendUser, $category = null)
    {
        ...
        if (FrontendUserSessionUtility::isUserLoggedIn($frontendUser)) {
            ...
            $this->signalSlotDispatcher->dispatch(
                __CLASS__,
                self::SIGNAL_AFTER_REGISTRATION_ENDED . ucfirst($category),
                [$frontendUser]
            );
     ...
     *
     * @deprecated Use Register/FrontendUserRegister->setUserGroups instead
     *
     * @param FrontendUser $frontendUser
     ...
     * @throws \TYPO3\CMS\Extbase\Configuration\Exception\InvalidConfigurationTypeException
     */
    public function setUserGroups(FrontendUser $frontendUser, $userGroups = '')
    {
        if (!$userGroups) {
            $settings = $this->getSettings();
        ...
        if ($frontendUser = $this->frontendUserRepository->findByUidAlsoInactiveNonGuest($register->getUser())) {
            ...
            if ($register->getTokenYes() == $tokenYes) {
                ...
                if ($category) {
                    ...
                    $this->signalSlotDispatcher->dispatch(
                        __CLASS__,
                        self::SIGNAL_AFTER_REGISTRATION_COMPLETED . ucfirst($category),
                        [$frontendUser, $register]
                    );
                    $data = [
                        'frontendUser' => $frontendUser,
